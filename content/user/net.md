...
1. Open the launcher.
1. Launch "Start multiplayer" (the first app on the list) by pressing `E` (`Enter` on the emulator).
1. Wait for all other devices to appear on the list.
1. Press `E` on both devices (in any order) to stop scanning.
1. Select "confirm" and press `E` on both devices (in any order) to confirm that the list of devices is complete.
